...
using Hurrah.GoogleDrive.Users;
using Castle.MicroKernel.Registration;
using Effort;
using EntityFramework.DynamicFilters;

namespace Hurrah.GoogleDrive.Tests
{
    public abstract class GoogleDriveTestBase : AbpIntegratedTestBase<GoogleDriveTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected GoogleDriveTestBase()
        {
        ...
        protected void UsingDbContext(Action<GoogleDriveDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<GoogleDriveDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<GoogleDriveDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<GoogleDriveDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<GoogleDriveDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GoogleDriveDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<GoogleDriveDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GoogleDriveDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<GoogleDriveDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<GoogleDriveDbContext, Task<T>> func)
        {
            T result;
